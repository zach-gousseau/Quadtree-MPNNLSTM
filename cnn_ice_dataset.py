...
class CNNIceDataset(IceDataset):
    """
    CNN version of IceDataset that doesn't use graph structures.
    Inherits from IceDataset but overrides methods to keep data in image format.
    """
    def __init__(self, 
                 ds, 
                 years, 
        ...
        # Call the parent get_xy method but without graph_structure
        self.x, self.y, self.launch_dates = self.get_xy_cnn(
            ds, 
            years,
            month, 
            input_timesteps, 
    ...
    def get_xy_cnn(self, ds, years, month, input_timesteps, output_timesteps, x_vars=None, y_vars=None, y_binary_thresh=None, mask=None, cache_dir=None):
        """
        CNN version of get_xy that doesn't use graph structures
        """
